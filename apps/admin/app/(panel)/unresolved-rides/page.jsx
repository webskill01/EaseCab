...
import { useUnresolvedRides } from '@/features/unresolved-rides/hooks/useUnresolvedRides'
import { UnresolvedRideRow } from '@/features/unresolved-rides/components/UnresolvedRideRow'

export default function UnresolvedRidesPage() {
  const { items, total, page, setPage, isLoading, isError, action } = useUnresolvedRides()

  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-ec-ink">
          Unresolved Rides <span className="text-sm font-normal text-ec-ink60">({total})</span>
        </h1>
      </div>
      <p className="mt-1 text-sm text-ec-ink60">Live bot rides missing a pickup or drop city. Set the right city, or hide the ride.</p>
      ...
      <div className="mt-4 flex flex-col gap-3">
        {items.map((row) => (
          <UnresolvedRideRow
            key={row.id}
            row={row}
            onAction={(id, act, side, cityId) => action.mutate({ id, action: act, side, cityId })}
          />
        ))}
      </div>
