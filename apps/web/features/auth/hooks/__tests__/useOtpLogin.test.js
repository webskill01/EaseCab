...
describe('useOtpLogin', () => {
  ...
  it('otp → perms → done for a new user', async () => {
    ...
    expect(verifyOtp).toHaveBeenCalledWith({ idToken: 'id-token' })
  ...
  it('server channel (2Factor): skips Firebase and verifies with phone + code', async () => {
    requestOtp.mockResolvedValue({ sent: true, channel: 'server' })
    verifyOtp.mockResolvedValue({ user: { id: 'u1' }, isNewUser: false })
    const { result } = renderHook(() => useOtpLogin())
    await act(async () => { await result.current.submitPhone('9876543210') })
    expect(sendOtp).not.toHaveBeenCalled()
    ...
    expect(verifyOtp).toHaveBeenCalledWith({ phone: '+919876543210', otp: '123456' })
    expect(replace).toHaveBeenCalledWith('/feed')
  })
})
