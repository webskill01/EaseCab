...
import { ChevronLeft, Shield, Pin, Steer, VehicleIcon, Globe, List, User } from '@/components/ui/icons'
...
export function PosterProfileScreen({ userId }) {
  ...
  const back = (
    <header className="flex items-center gap-2.5 border-b border-ec-line bg-white px-3 py-2.5">
      ...
      <span className="text-[15px] font-extrabold text-ec-ink">{t('poster.title')}</span>
    </header>
  )
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-ec-bg">
      ...
      <div className="-mt-7 px-4 pb-4">
        {/* Stats */}
        <div className="grid grid-cols-3 divide-x divide-y divide-ec-line overflow-hidden rounded-2xl border border-ec-line bg-white shadow-ec-card">
          <Stat icon={<Steer size={16} />} label={t('stats.experience')} value={p.experience != null ? `${p.experience} ${t('stats.years')}` : '—'} />
          <Stat icon={<Pin size={15} />} label={t('poster.stats.baseCity')} value={p.baseCity || '—'} />
          <Stat icon={<VehicleIcon vehicleKey={vehIconKeyOf(p.vehicleType)} size={16} />} label={t('stats.vehicle')} value={p.vehicleType || '—'} />
          <Stat icon={<Shield size={15} />} label={t('poster.stats.aadhaar')} value={statusLabel(v.aadhaarVerified)} />
          <Stat icon={<List size={15} />} label={t('poster.stats.license')} value={statusLabel(v.dlSubmitted)} />
          <Stat icon={<Globe size={15} />} label={t('poster.stats.languages')} value={String(p.languagesSpoken.length)} />
        </div>
