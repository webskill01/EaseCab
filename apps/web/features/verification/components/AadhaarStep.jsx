...
/** L1 step 1 — Aadhaar number entry (SCREENS §7). */
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (valid && !loading) onSubmit(num) }} className="flex flex-1 flex-col gap-4 p-5">
      ...
      <label className="flex flex-col gap-1.5">
        ...
        <input inputMode="numeric" value={num} onChange={(e) => setNum(e.target.value.replace(/\D/g, '').slice(0, 12))}
          aria-label={t('l1.aadhaarLabel')} className="h-12 rounded-xl border-[1.5px] border-ec-line bg-white px-3 text-[16px] font-bold tracking-wider text-ec-ink outline-none focus:border-ec-blue" />
