...
test('runTransitions runs all four jobs in order and returns the counts', async () => {
  ...
  assert.deepEqual(result, { booked: 1, hidden: 2, deleted: 3, fingerprintsPurged: 4 });
  assert.deepEqual(repository.order, ['markBooked', 'markHidden', 'hardDelete', 'purgeFingerprints']);
});
