...
test('parseEnv accepts a fully valid env and freezes the result', () => {
  ...
  assert.strictEqual(r.data.DATABASE_URL, BASE.DATABASE_URL);
});

test('a leftover WA_TARGET_GROUP_JID in .env is ignored (groups live in wa_groups now)', () => {
  const r = parseEnv({ ...BASE, WA_TARGET_GROUP_JID: '120363000000000000@g.us' });
  assert.strictEqual(r.success, true);
  assert.ok(!('WA_TARGET_GROUP_JID' in r.data));
});
